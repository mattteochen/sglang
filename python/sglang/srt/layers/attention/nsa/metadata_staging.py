...
class PrefillNsaMetadataBuffers:
    ...
    # (max_num_tokens, index_topk) int32 — scratch output for
    # `topk_construct_k_only_pcg`. Stable address bypasses the CUDA-graph pool's
    # per-capture lifetime tracking, which under TP>1 + many in-graph
    # allocations otherwise reuses the slab before FMHA reads topk_indices.
...
def allocate_prefill_nsa_metadata_buffers(
    ...
    return PrefillNsaMetadataBuffers(
        ...
        topk_result=_alloc_static((max_num_tokens, index_topk), device),
    )
